...
class AddTokenRequest(BaseModel):
    ...
    proxy_url: Optional[str] = Field(default=None, alias="proxyUrl")  # Proxy URL (optional)
...
class UpdateTokenRequest(BaseModel):
    ...
    proxy_url: Optional[str] = Field(default=None, alias="proxyUrl")  # Proxy URL
    remark: Optional[str] = None
...
class ImportTokenItem(BaseModel):
    ...
    proxy_url: Optional[str] = Field(default=None, alias="proxyUrl")  # Proxy URL (optional, for compatibility)
...
class UpdateAdminConfigRequest(BaseModel):
    ...
    impersonate_preset: Optional[str] = "safari_ios"
...
async def st_to_at(request: Request, body_req: ST2ATRequest, token: str = Depends(verify_admin_token)):
    """Convert Session Token to Access Token (only convert, not add to database)"""
    try:
        # Capture raw body to debug what frontend is actually sending
        raw_body = await request.json()
        print(f"DEBUG: Raw Request Body Keys: {list(raw_body.keys())}")
        print(f"DEBUG: Raw Request Body Content: {raw_body}")
        
        # Determine effective proxy url
        # Priority: 1. validated field (proxy_url/proxyUrl) 2. raw 'proxy' field 3. raw 'proxyUri' field
        effective_proxy = body_req.proxy_url
        if not effective_proxy:
            effective_proxy = raw_body.get('proxy') or raw_body.get('proxyUri')
            if effective_proxy:
                print(f"DEBUG: Found proxy in fallback field: {effective_proxy}")

        from ..core.logger import debug_logger
        debug_logger.log_info(f"API Request st_to_at: proxy_url={effective_proxy}")
        
        result = await token_manager.st_to_at(body_req.st, proxy_url=effective_proxy)
        return {
            "success": True,
...
async def import_tokens(request: ImportTokensRequest, token: str = Depends(verify_admin_token)):
    ...
    for import_item in request.tokens:
        try:
            ...
            if existing_token:
                # Check if proxy_url is used by other tokens (excluding self)
                if import_item.proxy_url:
                    existing_proxy = await db.get_token_by_proxy(import_item.proxy_url)
                    if existing_proxy and existing_proxy.id != existing_token.id:
                        raise ValueError(f"Proxy URL '{import_item.proxy_url}' is already used by another token ({existing_proxy.email})")
                ...
                # Check if proxy_url is used by any token
                if import_item.proxy_url:
                    existing_proxy = await db.get_token_by_proxy(import_item.proxy_url)
                    if existing_proxy:
                        raise ValueError(f"Proxy URL '{import_item.proxy_url}' is already used by another token ({existing_proxy.email})")
    ...
    if failed_count > 0:
        # If any item failed (e.g. duplicate proxy), return 400 Bad Request
        # The body will still contain detailed results so client knows which one failed
...
async def get_admin_config(token: str = Depends(verify_admin_token)) -> dict:
    ...
    return {
        ...
        "impersonate_preset": admin_config.impersonate_preset
    }
...
async def update_admin_config(
    ...
    try:
        ...
        if request.impersonate_preset:
            current_config.impersonate_preset = request.impersonate_preset
            config.set_impersonate_preset(request.impersonate_preset)
...
async def activate_sora2(
    ...
    try:
        ...
        # Activate Sora2
        result = await token_manager.activate_sora2_invite(token_obj.token, invite_code, token_id=token_obj.id, proxy_url=token_obj.proxy_url)

        if result.get("success"):
            # Get new invite code after activation
            sora2_info = await token_manager.get_sora2_invite_code(token_obj.token, token_id=token_obj.id)

            # Get remaining count
            sora2_remaining_count = 0
            try:
                remaining_info = await token_manager.get_sora2_remaining_count(token_obj.token, token_id=token_obj.id)
...
async def enable_all_tokens_route(token: str = Depends(verify_admin_token)):
    """Enable all tokens"""
    try:
        count = await token_manager.enable_all_tokens()
        return {"success": True, "message": f"已启用 {count} 个 Token"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@router.post("/api/tokens/disable-all")
async def disable_all_tokens_route(token: str = Depends(verify_admin_token)):
    """Disable all tokens"""
    try:
        count = await token_manager.disable_all_tokens()
        return {"success": True, "message": f"已禁用 {count} 个 Token"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@router.post("/api/tokens/usage/reset")
async def reset_usage_counts_route(token: str = Depends(verify_admin_token)):
...
@router.get("/api/scheduling/config")
async def get_scheduling_config(token: str = Depends(verify_admin_token)):
    """Get scheduling configuration"""
    try:
        mode = await token_manager.get_scheduling_mode()
        return {"success": True, "mode": mode}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@router.post("/api/scheduling/config")
async def update_scheduling_config(request: UpdateSchedulingRequest, token: str = Depends(verify_admin_token)):
    """Update scheduling configuration"""
    try:
        if request.mode not in ["random", "round_robin"]:
            raise ValueError("Invalid scheduling mode. Must be 'random' or 'round_robin'")
        await token_manager.update_scheduling_mode(request.mode)
        return {"success": True, "message": "调度模式已更新"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@router.post("/api/admin/fix-db")
async def fix_database(token: str = Depends(verify_admin_token)):
    """Force database migration check"""
    try:
        # Get config from setting.toml (we can't easily access global config here, but None is fine for adding columns)
