...
class TokenManager:
    ...
    async def get_user_info(self, access_token: str, token_id: Optional[int] = None, proxy_url: Optional[str] = None) -> dict:
        ...
        async with AsyncSession() as session:
            ...
            kwargs = {
                "headers": headers,
                "timeout": 30,
                "impersonate": config.impersonate_preset  # 自动生成 User-Agent 和浏览器指纹
            }
    ...
    async def check_username_available(self, access_token: str, username: str, token_id: Optional[int] = None, proxy_url: Optional[str] = None) -> bool:
        ...
        Args:
            ...
            proxy_url: Proxy URL to use directly (optional, takes precedence over token_id)

        Returns:
            True if username is available, False otherwise
        """
        proxy_url = await self.proxy_manager.get_proxy_url(token_id) if proxy_url is None else proxy_url
        ...
        async with AsyncSession() as session:
            ...
            kwargs = {
                ...
                "timeout": 30,
                "impersonate": config.impersonate_preset
            }
    ...
    async def set_username(self, access_token: str, username: str, token_id: Optional[int] = None, proxy_url: Optional[str] = None) -> dict:
    ...
    async def activate_sora2_invite(self, access_token: str, invite_code: str, token_id: Optional[int] = None, proxy_url: Optional[str] = None) -> dict:
        ...
        async with AsyncSession() as session:
            ...
            kwargs = {
                ...
                "timeout": 30,
                "impersonate": config.impersonate_preset  # 使用 chrome120 让库自动处理 UA 等头
            }
    ...
    async def st_to_at(self, session_token: str, proxy_url: Optional[str] = None) -> dict:
        ...
        # 优先使用传入的 proxy_url，否则根据策略获取
        effective_proxy_url = await self.proxy_manager.get_proxy_url(proxy_url=proxy_url)
        if proxy_url:
            debug_logger.log_info(f"[ST_TO_AT] 指定使用独立代理: {proxy_url}")
        else:
             debug_logger.log_info(f"[ST_TO_AT] 使用分配代理: {effective_proxy_url}")
        ...
        async with AsyncSession() as session:
            ...
            if effective_proxy_url:
                kwargs["proxy"] = effective_proxy_url
                debug_logger.log_info(f"[ST_TO_AT] 实际请求代理: {effective_proxy_url}")
    ...
    async def rt_to_at(self, refresh_token: str, client_id: Optional[str] = None, proxy_url: Optional[str] = None) -> dict:
        ...
        effective_proxy_url = await self.proxy_manager.get_proxy_url(proxy_url=proxy_url)
        if proxy_url:
             debug_logger.log_info(f"[RT_TO_AT] 指定使用独立代理: {proxy_url}")
        else:
             debug_logger.log_info(f"[RT_TO_AT] 使用分配代理: {effective_proxy_url}")
        ...
        async with AsyncSession() as session:
            ...
            if effective_proxy_url:
                kwargs["proxy"] = effective_proxy_url
                debug_logger.log_info(f"[RT_TO_AT] 实际请求代理: {effective_proxy_url}")
    ...
    async def test_token(self, token_id: int) -> dict:
        ...
        try:
            ...
            user_info = await self.get_user_info(token_data.token, token_id=token_id)

            # Refresh Sora2 invite code and counts
            sora2_info = await self.get_sora2_invite_code(token_data.token, token_id=token_id)
            sora2_supported = sora2_info.get("supported", False)
            sora2_invite_code = sora2_info.get("invite_code")
            sora2_redeemed_count = sora2_info.get("redeemed_count", 0)
            sora2_total_count = sora2_info.get("total_count", 0)
            ...
            if sora2_supported:
                try:
                    remaining_info = await self.get_sora2_remaining_count(token_data.token, token_id=token_id)
                    if remaining_info.get("success"):
                        sora2_remaining_count = remaining_info.get("remaining_count", 0)
                except Exception as e:
                    print(f"Failed to get Sora2 remaining count: {e}")
    ...
    async def auto_refresh_expiring_token(self, token_id: int) -> bool:
        ...
        try:
            ...
            debug_logger.log_info(f"  - 单独代理: {token_data.proxy_url if token_data.proxy_url else '无'}")
    ...
    async def increment_usage_count(self, token_id: int):
        """Increment usage count for round-robin scheduling"""
        try:
            await self.db.increment_usage_count(token_id)
        except Exception as e:
            debug_logger.log_info(f"Failed to increment usage count: {e}")

    async def reset_all_usage_counts(self):
        """Reset all usage counts"""
        try:
            await self.db.reset_all_usage_counts()
        except Exception as e:
            debug_logger.log_info(f"Failed to reset usage counts: {e}")

    async def enable_all_tokens(self) -> int:
        """Enable all tokens"""
        try:
            return await self.db.enable_all_tokens()
        except Exception as e:
            debug_logger.log_info(f"Failed to enable all tokens: {e}")
            return 0

    async def disable_all_tokens(self) -> int:
        """Disable all tokens"""
        try:
            return await self.db.disable_all_tokens()
        except Exception as e:
            debug_logger.log_info(f"Failed to disable all tokens: {e}")
            return 0

    async def get_scheduling_mode(self) -> str:
        """Get current scheduling mode"""
        try:
            config = await self.db.get_admin_config()
            return config.scheduling_mode
        except Exception as e:
            debug_logger.log_info(f"Failed to get scheduling_mode: {e}")
            return "random"

    async def update_scheduling_mode(self, mode: str):
        """Update scheduling mode"""
        # Don't swallow exception here, let it bubble up to API
        await self.db.update_scheduling_mode(mode)
