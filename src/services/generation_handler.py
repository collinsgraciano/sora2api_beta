...
class GenerationHandler:
    ...
    async def handle_background_generation(self, task_id: str, token_obj: Any, is_video: bool, 
                                         prompt: str, model: str, start_time: float, log_id: Optional[int]):
        """Handle background generation task (polling and cleanup)"""
        try:
            # Poll for results
            async for _ in self._poll_task_result(task_id, token_obj.token, is_video, stream=False, prompt=prompt, token_id=token_obj.id):
                pass
            ...
            debug_logger.log_error(f"Background task {task_id} failed: {str(e)}")
            
            # Release resources on error
            if not is_video and token_obj:
                await self.load_balancer.token_lock.release_lock(token_obj.id)
                if self.concurrency_manager:
                    await self.concurrency_manager.release_image(token_obj.id)
            elif is_video and token_obj and self.concurrency_manager:
                await self.concurrency_manager.release_video(token_obj.id)
    ...
    async def submit_background_task(self, model: str, prompt: str,
        ...
        if is_video and self.concurrency_manager:
            ...
            if not concurrency_acquired:
                # Cleanup handled in finally/except block if needed, but here we just raise
                # (Caller technically hasn't started bg task yet, so just raise)
                raise Exception(f"Failed to acquire concurrency slot for token {token_obj.id}")
        ...
        try:
            ...
            if is_video:
                # Handle remix/storyboard logic similar to handle_generation
                # Note: Simplified for now, assuming standard generation or storyboard
            ...
            asyncio.create_task(self.handle_background_generation(
                task_id, token_obj, is_video, prompt, model, start_time, log_id
            ))
    ...
    async def _download_file(self, url: str, token_id: Optional[int] = None) -> bytes:
        ...
        proxy_url = await self.load_balancer.proxy_manager.get_proxy_url(token_id)
                                        ...
                                        if config.cache_enabled:
                                            try:
                                                ...
                                                try:
                                                    ...
                                                    await self.sora_client.delete_post(post_id, token, token_id=token_id)
    ...
    async def _handle_character_creation_only(self, video_data, model_config: Dict) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            if isinstance(video_data, str):
                ...
                video_bytes = await self._download_file(video_data, token_id=token_obj.id)
            else:
            ...
            cameo_id = await self.sora_client.upload_character_video(video_bytes, token_obj.token, token_id=token_obj.id)
            ...
            cameo_status = await self._poll_cameo_status(cameo_id, token_obj.token, token_id=token_obj.id)
            ...
            avatar_data = await self.sora_client.download_character_image(profile_asset_url, token_id=token_obj.id)
            ...
            await self.sora_client.set_character_public(cameo_id, token_obj.token, token_id=token_obj.id)
        ...
        try:
            ...
            if character_id:
                try:
                    ...
                    await self.sora_client.delete_character(character_id, token_obj.token, token_id=token_obj.id)
    ...
    async def _poll_cameo_status(self, cameo_id: str, token: str, token_id: Optional[int] = None, timeout: int = 600, poll_interval: int = 5) -> Dict[str, Any]:
        ...
        for attempt in range(max_attempts):
            ...
            try:
                status = await self.sora_client.get_cameo_status(cameo_id, token, token_id=token_id)
