...
class Database:
    ...
    async def _ensure_config_rows(self, db, config_dict: dict = None):
        ...
        if count[0] == 0:
            ...
            if config_dict:
                ...
                scheduling_mode = admin_config.get("scheduling_mode", "random")
                impersonate_preset = admin_config.get("impersonate_preset", "safari_ios")

            await db.execute("""
                INSERT INTO admin_config (id, admin_username, admin_password, api_key, error_ban_threshold, scheduling_mode, impersonate_preset)
                VALUES (1, ?, ?, ?, ?, ?, ?)
            """, (admin_username, admin_password, api_key, error_ban_threshold, scheduling_mode, impersonate_preset))
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if await self._table_exists(db, "admin_config"):
                columns_to_add = [
                    ...
                    ("scheduling_mode", "TEXT DEFAULT 'random'"),
                    ("impersonate_preset", "TEXT DEFAULT 'safari_ios'"),
                ]
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                INSERT INTO tokens (token, email, username, name, st, rt, client_id, proxy_url, remark, expiry_time, is_active,
                                   ...
                                   image_enabled, video_enabled, image_concurrency, video_concurrency, usage_count)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (token.token, token.email, "", token.name, token.st, token.rt, token.client_id, token.proxy_url,
                  ...
                  token.image_concurrency, token.video_concurrency, 0))
            await db.commit()
    ...
    async def get_token_by_proxy(self, proxy_url: str) -> Optional[Token]:
        """Get token by proxy URL"""
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
    ...
    async def increment_usage_count(self, token_id: int):
        """Increment round-robin usage count"""
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("UPDATE tokens SET usage_count = usage_count + 1 WHERE id = ?", (token_id,))
            await db.commit()

    async def reset_all_usage_counts(self):
        """Reset all usage counts to 0"""
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("UPDATE tokens SET usage_count = 0")
            await db.commit()

    async def update_scheduling_mode(self, mode: str):
        """Update scheduling mode"""
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("UPDATE admin_config SET scheduling_mode = ? WHERE id = 1", (mode,))
            if cursor.rowcount == 0:
                await db.execute("INSERT INTO admin_config (id, scheduling_mode) VALUES (1, ?)", (mode,))
            await db.commit()
    ...
    async def update_admin_config(self, config: AdminConfig):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("""
                UPDATE admin_config
                SET admin_username = ?, admin_password = ?, api_key = ?, error_ban_threshold = ?, impersonate_preset = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = 1
            """, (config.admin_username, config.admin_password, config.api_key, config.error_ban_threshold, config.impersonate_preset))
            await db.commit()
    ...
    async def enable_all_tokens(self) -> int:
        """Enable all tokens"""
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("UPDATE tokens SET is_active = 1, cooled_until = NULL, sora2_cooldown_until = NULL")
            await db.commit()
    ...
    async def disable_all_tokens(self) -> int:
        """Disable all tokens"""
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("UPDATE tokens SET is_active = 0")
            await db.commit()
            return cursor.rowcount
